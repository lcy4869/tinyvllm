...
@triton.jit
def store_kvcache_kernel(k_ptr, k_stride, v_ptr, v_stride, k_cache, v_cache, slot_mapping_ptr, D):
    idx = tl.program_id(0)
    key_offset = idx * k_stride + tl.arange(0, D)
    k = tl.load(k_ptr + key_offset)
    value_offset = idx * v_stride + tl.arange(0, D)
    v = tl.load(k_ptr + value_offset)
    slot = tl.load(slot_mapping_ptr + idx)
    cache_offset =  slot*D + tl.arange(0, D)
    tl.load(k_cache + cache_offset, k)
    tl.load(v_cache + cache_offset, v)
...
class Attention(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        store_kv(k, v, k_cache, v_cache, context.slot_mapping)
        block_tables = context.block_tables
        k_cache, v_cache = self.k_cache, self.v_cache
        if context.is_prefill:
            if context.block_table is not None:
                k, v = k_cache, v_cache
            ...
            o = flash_attn_varlen_func(q, k, v, 
                                       ...
                                       causal=True, block_table=context.block_table)
        else:
            o = flash_attn_with_kvcache(q, k_cache, v_cache, cache_seqlens=context.context_lens, block_table=block_tables)
        return o
